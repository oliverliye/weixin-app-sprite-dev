...
var $, Config, S, Wxss, _, fs, getPriority, parseClass, path, setImport, wxdbrn, wxssrn;
...
setImport = function(basePath, data) {
  var i, improts, item, len, pathName, subData;
  improts = data.match(/@import\s+("|')(.+?)("|')\s*;/g);
  if (!improts) {
    return data;
  }
  for (i = 0, len = improts.length; i < len; i++) {
    item = improts[i];
  ...
  Wxss.prototype.setToWxml = function(wxml) {
    ...
    return $('*', $wxml).each((function(_this) {
      return function(index, elem) {
        var $elem, cls, data, j, len1, ref1, style, styleData;
        $elem = $(elem);
